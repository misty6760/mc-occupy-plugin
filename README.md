...
**웅크리기(Shift)** 상태에서 왼손에 아이템을 들고 **F키**를 눌러 교환:
...
- **쿨다운 시스템**: 요청 쿨다운 10초, 텔레포트 성공 후 5분 쿨다운
...
```
웅크리기(Shift) 상태에서 왼손에 교환할 아이템을 들고 F키를 누르세요
```
...
1. **기본 4개 모두 점령**: 기본 점령지 4곳을 모두 점령하면 보너스 1점으로 승리
2. **기본 2개 + 중앙**: 기본 점령지 2개와 중앙을 점령하면 보너스 1점으로 승리
3. **팀 협력**: `/tpa`로 팀원끼리 이동하여 점령지 방어/공격 (쿨타임 주의)
4. **교환 시스템**: 왼손에 청금석/철을 들고 웅크리기 + F키로 경험치/식량 교환
...
- 🐛 교환 시스템 수정 (왼손 아이템만 사용)
- 🐛 승리 조건 수정 (5점 승리, 보너스 점수 시스템)
- 🐛 TPA 쿨타임 시스템 개선 (요청 10초, 텔레포트 5분)
- 🐛 점령지 파티클 효과 제거
...
### 주요 변경사항
- ✅ 승리 조건 변경: 4점 → **5점** (보너스 점수 시스템 추가)
- ✅ 교환 시스템 개선: **왼손 아이템만** 사용하도록 수정
- ✅ TPA 쿨타임 시스템: 요청 쿨다운 **10초**, 텔레포트 쿨다운 **5분**
- ✅ 점령지 파티클 효과 제거
- ✅ 교환 시스템 웅크리기 조건 추가
